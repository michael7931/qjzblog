    ...
    <form class="form-horizontal " action="{{ url('admin/article/update', [$article->id]) }}" method="post">
        ...
        <table class="table table-striped table-bordered table-hover">
            ...
            <tr>
                ...
                <td>
                    @foreach($tag as $v)
                        {{ $v['name'] }}<input class="qjz-icheck" type="checkbox" name="tag_ids[]" value="{{ $v['id'] }}" @if(in_array($v['id'], $article->tag_ids)) checked="checked" @endif> &emsp;
                    @endforeach
                </td>
            </tr>
            <tr>
...
@section('js')
    ...
    <script>
        ...
        $(function() {
            ...
            testEditor = editormd("qjz-content", {
                width     : "100%",
                height    : 720,
                toc       : true,
